    ...
    public partial class GraficForm : Form {
        ...
        private void panel1_Paint(object sender, PaintEventArgs e) {
            ...
            for (int i = 0; i < vector_km.Length; i++) {
                vector_km[i] = biciclete[i].KmParcursi;
            }

            Graphics gr = e.Graphics;
            Rectangle container = new Rectangle(panel1.ClientRectangle.X + margine, panel1.ClientRectangle.Y
                + 2 * margine, panel1.ClientRectangle.Width - 2 * margine, panel1.ClientRectangle.Height - 3 * margine);
            gr.DrawRectangle(new Pen(Color.Red), container);
